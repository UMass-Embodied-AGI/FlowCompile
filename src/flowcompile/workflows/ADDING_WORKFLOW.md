...
workflow lives in its own folder under `src/flowcompile/workflows/` and is
...
Tip: You can copy the template at `src/flowcompile/workflows/template/` to get started.
...
`src/flowcompile/workflows/dsl_registry.py`

```python
from flowcompile.workflows.myworkflow.workflow import MyWorkflowDSL
...
- `_preprocess_query` in `src/flowcompile/dsl/runtime.py`
- `_build_trace_*` in `src/flowcompile/dsl/runtime.py`
