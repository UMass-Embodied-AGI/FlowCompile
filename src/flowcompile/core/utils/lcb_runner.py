...
def timeout_handler(signum, frame):
    # Clear the alarm before raising exception to prevent getting stuck
    signal.alarm(0)
...
def grade_call_based(
    ...
    if compiled_sol is None:
        return [-4], {
            "error_code": -4,
            "error_message": "Compilation returned no module",
        }
    ...
    if method is None:
        return [-4], {
            "error_code": -4,
            "error_message": f"Entry point '{fn_name}' not found in submission",
        }

    # Parse inputs - each input string may contain multiple lines, each line is a JSON value
    parsed_inputs = []
    for inputs in all_inputs:
        try:
            # Split by newlines and parse each non-empty line as JSON
            lines = [line.strip() for line in inputs.split("\n") if line.strip()]
            parsed_lines = []
            for line in lines:
                try:
            ...
            parsed_inputs.append(parsed_lines)
        except Exception as e:
            # Fallback: treat the whole input as a single string
            parsed_inputs.append([inputs])
    
    all_inputs = parsed_inputs

    # Parse outputs - each output is typically a single JSON value
    parsed_outputs = []
...
def grade_stdio(
    ...
    if method is None:
        return [-4], {
            "error_code": -4,
            "error_message": "Entry point 'wrapped_function' not found; ensure code writes to stdout",
        }
...
def run_test(sample, test=None, debug=False, timeout=6):
    ...
    # This is safe in multiprocessing context as each test runs in its own process
    reliability_guard()
    ...
    if test is None:
        ...
        if debug:
            ...
            print(which_type)
            print(test)
            print("all_inputs:", in_outs["inputs"])
            print("all_outputs:", in_outs["outputs"])
            print(method_name)

        if which_type == CODE_TYPE.call_based:
            signal.alarm(timeout)
            try:
                graded = grade_call_based(
                    code=test,
                ...
                if graded is None:
                    return [-4], {
                        "error_code": -4,
                        "error_message": "Testing produced no result (possible compile or entry-point failure)",
                    }
                results, metadata = graded
