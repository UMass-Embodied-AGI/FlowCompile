...
from flowcompile.benchmarks.registry import register_benchmark
...
class HotpotQABenchmark(BaseBenchmark):
    ...
    ALIASES = ["hotpotqa", "HotpotQA", "HOTPOTQA"]
    ...
    def normalize_answer(self, s: str | None) -> str:
    ...
    async def evaluate_problem(self, problem: dict, graph: Callable) -> Tuple[str, str, str, str, float]:
        ...
        try:
            ...
            return input_text, context_str, output, expected_output, score

        except Exception as e:
            logger.info(f"Maximum retries reached. Skipping this sample. Error: {e}")
            return input_text, context_str, str(e), expected_output, 0.0

    def get_result_columns(self) -> List[str]:
        return ["question", "context", "prediction", "expected_output", "score"]

    @classmethod
    def score_from_result(cls, result: Tuple[str, ...]) -> float:
        return float(result[-1])

    @classmethod
    def result_key(cls, result: Tuple[str, ...]) -> str:
        return str(result[0]) if result else ""

    @classmethod
    def trace_key(cls, trace: dict) -> str:
        orig = trace.get("metadata", {}).get("original_sample", {}) or {}
        question = orig.get("question") or trace.get("question")
