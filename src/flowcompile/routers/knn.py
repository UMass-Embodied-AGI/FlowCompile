...
class _NumpyCosineNearestNeighbors:
    """Small cosine KNN fallback used when scikit-learn is unavailable."""

    def __init__(self, n_neighbors: int, metric: str = 'cosine'):
        if metric != 'cosine':
            raise ValueError("NumPy nearest-neighbor fallback only supports cosine metric")
    ...
    def kneighbors(self, queries: np.ndarray, n_neighbors: Optional[int] = None):
        if self._vectors is None:
            raise ValueError("Nearest-neighbor index has not been fitted")

        query_vectors = np.asarray(queries, dtype=float)
        neighbor_count = min(n_neighbors or self.n_neighbors, len(self._vectors))
        vector_norms = np.linalg.norm(self._vectors, axis=1)
        query_norms = np.linalg.norm(query_vectors, axis=1, keepdims=True)
        denominator = np.maximum(query_norms * vector_norms.reshape(1, -1), 1e-12)
        similarities = (query_vectors @ self._vectors.T) / denominator
        all_distances = 1.0 - similarities
        indices = np.argsort(all_distances, axis=1)[:, :neighbor_count]
        distances = np.take_along_axis(all_distances, indices, axis=1)
        return distances, indices


def _make_nearest_neighbors(n_neighbors: int, metric: str = 'cosine'):
    try:
        from sklearn.neighbors import NearestNeighbors

        return NearestNeighbors(n_neighbors=n_neighbors, metric=metric)
    except ModuleNotFoundError:
        logger.warning("scikit-learn is unavailable; using NumPy cosine KNN fallback")
        return _NumpyCosineNearestNeighbors(n_neighbors=n_neighbors, metric=metric)
...
class KNNRouter(Router):
    ...
    def _build_knn_index(self):
        ...
        # Build KNN index
        self.knn_index = _make_nearest_neighbors(
