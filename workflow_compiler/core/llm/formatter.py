...
from workflow_compiler.core.utils.sanitize import sanitize
...
class CodeFormatter(BaseFormatter):
    ...
    def validate_response(self, response: str) -> Tuple[bool, Union[Dict[str, str], str, None]]:
        ...
        try:
            ...
            # with a timeout to prevent hanging. Avoid waiting on shutdown to prevent
            # executor.join from blocking the event loop when interrupted.
            ...
            try:
                ...
                logger.warning(f"Code sanitization error: {sanitize_error}, returning original code")
                sanitized_code = code
            finally:
                try:
